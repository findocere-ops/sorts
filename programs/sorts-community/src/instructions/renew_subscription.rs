...
impl RenewSubscription {
    pub fn renew_subscription(&mut self, level: u8) -> Result<(), ProgramError> {
        ...
        if self.subscription.subscriber != *self.subscriber.address() {
        ...
        let subscriber_addr = self.subscription.subscriber;
        let tier_commitment = self.subscription.tier_commitment;
