...
- `NEXT_PUBLIC_SORTS_FACTORY_ADDRESS` — deployed SortsFactory address
- `NEXT_PUBLIC_API_URL` — backend URL
- `TELEGRAM_BOT_TOKEN` — grammy bot token
- `DATABASE_PATH` — path to SQLite file (default `./data/sorts.db`)
- `PRIVATE_KEY` — deployer wallet private key (contracts only, never commit)
- `IEXEC_PRIVATE_KEY` — backend iExec operator key for DataProtector
- `IEXEC_SORTS_IAPP_ADDRESS` — iExec app authorized to process protected content
...
- Backend: `cd backend && pnpm test` (TypeScript compile check)
- Frontend: `cd frontend && pnpm test` (Next.js production build smoke test)
